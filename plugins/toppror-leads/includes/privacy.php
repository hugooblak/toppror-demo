...
add_filter(
	...
	function ( $exporters ) {
		$exporters['toppror-leads'] = array(
			'exporter_friendly_name' => __( 'Quote requests', 'toppror-leads' ),
			'callback'               => function ( $email ) {
				$items = array();
				foreach ( npl_leads_by_email( $email ) as $lead ) {
					$data = array( array( 'name' => __( 'Received', 'toppror-leads' ), 'value' => $lead->post_date ) );
					...
					$items[] = array(
						'group_id'    => 'toppror-leads',
						'group_label' => __( 'Quote requests', 'toppror-leads' ),
...
add_filter(
	...
	function ( $erasers ) {
		$erasers['toppror-leads'] = array(
			'eraser_friendly_name' => __( 'Quote requests', 'toppror-leads' ),
			'callback'             => function ( $email ) {
				$removed = false;
				foreach ( npl_leads_by_email( $email ) as $lead ) {
