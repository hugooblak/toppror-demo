...
add_action(
	...
	function () {
		register_post_type(
			...
			array(
				'labels'          => array(
					'name'          => __( 'Leads', 'toppror-leads' ),
					'singular_name' => __( 'Lead', 'toppror-leads' ),
					'edit_item'     => __( 'Förfrågan', 'toppror-leads' ),
					'search_items'  => __( 'Search leads', 'toppror-leads' ),
					'not_found'     => __( 'Inga förfrågningar än. Skicka formuläret så dyker den upp här.', 'toppror-leads' ),
					'all_items'     => __( 'Alla förfrågningar', 'toppror-leads' ),
				),
...
add_filter(
	...
	function () {
		return array(
			...
			'title'        => __( 'Name', 'toppror-leads' ),
			'npl_phone'    => __( 'Phone', 'toppror-leads' ),
			'npl_service'  => __( 'Needs', 'toppror-leads' ),
			'npl_timeline' => __( 'When', 'toppror-leads' ),
			'npl_zip'      => __( 'ZIP', 'toppror-leads' ),
			'npl_source'   => __( 'Source', 'toppror-leads' ),
			'npl_status'   => __( 'Status', 'toppror-leads' ),
			'date'         => __( 'Received', 'toppror-leads' ),
		);
	}
);
...
function npl_source_summary( $post_id ) {
	...
	return __( 'Direct', 'toppror-leads' );
}
...
add_action(
	...
	function () {
		add_meta_box( 'npl_details', __( 'What they told us', 'toppror-leads' ), 'npl_details_box', 'npl_lead', 'normal', 'high' );
		add_meta_box( 'npl_status', __( 'Status', 'toppror-leads' ), 'npl_status_box', 'npl_lead', 'side', 'high' );
	}
);
...
function npl_details_box( $post ) {
	$rows = array(
		__( 'Name', 'toppror-leads' )     => get_post_meta( $post->ID, 'name', true ),
		__( 'Phone', 'toppror-leads' )    => get_post_meta( $post->ID, 'phone', true ),
		__( 'E-post', 'toppror-leads' )    => get_post_meta( $post->ID, 'email', true ),
		__( 'ZIP', 'toppror-leads' )      => get_post_meta( $post->ID, 'zip', true ),
		__( 'Needs', 'toppror-leads' )    => npl_label( 'service', get_post_meta( $post->ID, 'service', true ) ),
		__( 'When', 'toppror-leads' )     => npl_label( 'timeline', get_post_meta( $post->ID, 'timeline', true ) ),
		__( 'Source', 'toppror-leads' )   => npl_source_summary( $post->ID ),
	);
	...
	$rows[ __( 'Samtycke lämnat', 'toppror-leads' ) ] = get_post_meta( $post->ID, 'consent_at', true ) . ' — "' . get_post_meta( $post->ID, 'consent_text', true ) . '"';
...
function npl_status_box( $post ) {
	...
	echo '<label class="screen-reader-text" for="npl-status">' . esc_html__( 'Status', 'toppror-leads' ) . '</label><select id="npl-status" name="npl_status" style="width:100%">';
