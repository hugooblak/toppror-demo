 ...
 * or photo. They call to_lead() instead. To build a demo for a real firm, the
 ...
 *
 * With no lead-data.php present the defaults below render the fictional Topp-Rör AB.
 *
 * @package Topp-Rör AB
 */
 ...
 * @return string
 */
function to_sample_note() {
	return to_lead( 'ar_riktig' ) ? '' : ' <span class="to-sample">(platshållare)</span>';
}
 ...
 * @return string File name for to_lead_img().
 */
function to_lead_bild( $i ) {
	$bilder = (array) to_lead( 'bilder', array() );
 ...
 * @return string
 */
function to_ring_label() {
	$namn = to_lead( 'kontakt' );
	return $namn ? sprintf( 'Ring %s · %s', $namn, to_lead( 'telefon' ) ) : 'Ring ' . to_lead( 'telefon' );
}
