<?php
/**
 * Topp-Rör AB theme functions.
 *
 * Kept short on purpose. Design lives in theme.json, markup in templates and patterns.
 * Lead capture lives in the "Topp-Rör AB Leads" plugin, so leads survive a theme change.
 *
 * @package Topp-Rör AB
 */
 ...
 * Patterns read these through to_lead() so one demo can be rebuilt for any firm.
 */
...
add_action(
	'wp_enqueue_scripts',
	function () {
		wp_enqueue_style( 'toppror', get_stylesheet_uri(), array(), wp_get_theme()->get( 'Version' ) );
	}
);
add_action(
	'after_setup_theme',
...
add_action(
	'init',
	function () {
		register_block_style( 'core/paragraph', array( 'name' => 'eyebrow', 'label' => __( 'Eyebrow label', 'toppror' ) ) );
		register_block_style( 'core/list', array( 'name' => 'checklist', 'label' => __( 'Tick list', 'toppror' ) ) );

		register_block_pattern_category( 'toppror-sections', array( 'label' => __( 'Topp-Rör AB sections', 'toppror' ) ) );
		register_block_pattern_category( 'toppror-pages', array( 'label' => __( 'Topp-Rör AB page layouts', 'toppror' ) ) );
	}
);
